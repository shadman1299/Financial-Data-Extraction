...
output_path = r"C:\Users\hassa\Desktop\Financial Data Extraction\extracted_financial_data.json"
...
def extract_financial_entities(text):
    entities = {
        ...
        "Revenue from Operations": None,
        "Total Income": None,
        "Net Profit After Tax": None,
        "Earnings Per Share": None,
        "Declared Dividend": None,
    }
    ...
    revenue_match = re.search(r"Revenue from operations.*?([\d,]+\.?\d*)", text, re.IGNORECASE)
    if revenue_match:
        entities["Revenue from Operations"] = revenue_match.group(1)

    # Extract total income
    income_match = re.search(r"Total income.*?([\d,]+\.?\d*)", text, re.IGNORECASE)
    if income_match:
        entities["Total Income"] = income_match.group(1)

    # Extract net profit after tax
    net_profit_match = re.search(r"Net Profit after tax.*?([\d,]+\.?\d*)", text, re.IGNORECASE)
    if net_profit_match:
        entities["Net Profit After Tax"] = net_profit_match.group(1)

    # Extract earnings per share
    eps_match = re.search(r"Earnings per share.*?([\d,]+\.?\d*)", text, re.IGNORECASE)
    if eps_match:
        entities["Earnings Per Share"] = eps_match.group(1)

    # Extract declared dividend
    dividend_match = re.search(r"Dividend.*?([\d,]+\.?\d*)", text, re.IGNORECASE)
    if dividend_match:
        entities["Declared Dividend"] = dividend_match.group(1)
